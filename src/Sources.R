...
expanded_sources_trade <- expanded_sources_trade |>
  dplyr::mutate(Name = dplyr::if_else(Info_Format == "year", paste(Name, Year, sep = "_"), Name))

# Plot showing years covered by sources_trade
plot_sources_trade <- ggplot2::ggplot(expanded_sources_trade, ggplot2::aes(y = Trade, x = Year, fill = "lightblue")) +
...
plot_sources_trade_sources <- ggplot2::ggplot(expanded_sources_trade, ggplot2::aes(y = Trade, x = Year, fill = Name, alpha = .8)) +
  ggplot2::geom_tile() +
  # theme_minimal() +
...
plot_sources_trade_quality <- ggplot2::ggplot(expanded_sources_trade, ggplot2::aes(y = Trade, x = Year, alpha = .8)) +
  ggplot2::geom_tile(ggplot2::aes(fill = factor(Quality))) +
  ggplot2::theme_dark() +
  ggplot2::labs(title = "Source Availability by Country", fill = "Quality") +
  ggplot2::guides(alpha = "none") +
  ggplot2::scale_fill_manual(values = c("Bad" = "red", "Good" = "green", "Unknown" = "blue")) +
  ggplot2::facet_grid(ImpExp ~ Reporter, scales = "free", space = "free_x")
plot_sources_trade_quality

# Plot showing years covered by sources_trade, colored by scanned/not scanned
plot_sources_trade_scanned <- ggplot2::ggplot(expanded_sources_trade, ggplot2::aes(y = Trade, x = Year)) +
  ...
  ggplot2::scale_fill_manual(labels = c("0" = "No", "1" = "Yes"), values = c("0" = "red", "1" = "green")) +
  ggplot2::guides(alpha = "none") +
  ggplot2::facet_grid(ImpExp ~ Reporter, scales = "free", space = "free_x")
plot_sources_trade_scanned

# Plot showing years covered by sources_trade, colored by whether or not in SACO
plot_sources_trade_saco <- ggplot2::ggplot(expanded_sources_trade, ggplot2::aes(y = Trade, x = Year, alpha = .8)) +
...
sources_prod <- googlesheets4::read_sheet(sheet_url, sheet = "Final_Sources_Production")
